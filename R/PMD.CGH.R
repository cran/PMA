ChooseLambda1Lambda2 <- function(y,chrom=NULL){
...
CGH.SMD <- function(x,lam1,lam2,sumabsu,chrom,niter=20,v, trace){
  nas <- is.na(x)
  xoo <- x
  xoo[nas] <- mean.na(x)
  vold <- rnorm(length(v))
...
MultiCGH.SMD <- function(x,K,lam1,lam2,sumabsu,chrom,niter,v, trace){
  uans = vans = dans = NULL
  xres <- x
  for(k in 1:K){
    if(K==1) out <- CGH.SMD(xres,lam1,lam2,sumabsu,chrom,niter,v, trace=trace)
    if(K>1) out <- CGH.SMD(xres,lam1,lam2,sumabsu,chrom,niter,v[,k], trace=trace)
...
PMDL1FL <- function(x,K=1,sumabsu=5,lambda=NULL,chrom=NULL,niter=20, v=NULL, trace=TRUE, center=TRUE, rnames=NULL, cnames=NULL){
  call <- match.call()
  if(center){
    meanx <- mean.na(x)
    x <- x-meanx
  ...
  out <- MultiCGH.SMD(x,K=K,lam1=lambda,lam2=lambda,sumabsu=sumabsu,chrom=chrom,niter=niter, v=v, trace=trace)
  out$rnames <- rnames
  out$cnames <- cnames
  out$call <- call
  out$meanx <- meanx
